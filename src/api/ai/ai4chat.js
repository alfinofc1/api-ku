...
module.exports = function(app) {
    async function fetchAi4chat(text) {
        try {
            const url = `https://api.nekorinn.my.id/ai/llama-3.1-8b?text=${encodeURIComponent(text)}`;
    ...
    app.get('/ai/llama-3.1-8b', async (req, res) => {
        try {
            ...
            res.status(200).json({
                status: true,
                creator: "alfin",
                result: result.result || result.message || "Tidak ada respons dari llama-3.1-8b AI ðŸ¥º"
            });
        } catch (error) {
            res.status(500).json({
                status: false,
